...
------------------------------------------------------------------------------------------------
Ryan Yoseph (ryosep2)
Partners: Tony Xu (tonyx2), Ishaan Attri (iattri2)

In this MP, we were tasked with implementing a floor plan. This floor plan would optimize the layout of a circuit design
by parsing the tree, adding the modules to each leaf node and adding the cutlines to the internal nodes. In this MP, we used
a good amount of recursion to go through the tree and check nodes with. What I learned from this MP was how to properly go through a tree
using recursion, using pointers to structs, accessing elements inside a struct, and a more solid understanding of recursion in general.
This MP's output was a checker that would let us know what functions were correct and what functions were incorrect. It would tally up our points
and shoot us the sum at the bottom of the checker.
-------------------------------------------------------------------------------------------------
*/
#include "floorplan.h"
...
// In this function, we check if a node is a leaf node by assessing if the left and right pointers are NULL
int is_leaf_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
	if (ptr -> left == NULL && ptr -> right == NULL){				// if node has no children, its pointers have value NULL, and therefore are leaf nodes
		return 1;
	}
  return 0;
}
...
// this function is similar to the leaf node function but instead we want to check That
// the pointers to left and right are not NULL since internal nodes have children
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
	if (ptr -> left != NULL && ptr -> right != NULL){				// If a node is internal, its pointers
			return 1;
  }
		return 0;
}
...
// This function uses recursion to check if node b is inside a subtree rooted at a. We test this
// by setting our base case when the pointers are equal to each other, and also checking if b is not in the subtree.
// This check is done by seeing if b will ever point to NULL. Our recursive function has a and b-> parent as parameters.
// Every time we recursively call our function, we change the pointer b (in each respective function) to be pointing to its parent.
// We found manipulating b to be a lot easier than manipulating a to test this function.
...
// This function takes the dimensions of a respective module and flips the coordinates of it,
// which effectively just rotates the module 90 degrees.
...
// This function switches the cutline of a node to H if it is V and V if it is H.
...
// This function swaps the modules of nodes by changing the pointers of a and b to their respective
// modules and setting them equal to another.
...
// This function switches the nodes of two subtrees and also allocates the pointers respectively of their parents.
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
	...
	if(a->parent->right == a){
		if(b-> parent -> right == b){
			swapinner = a-> parent -> right;
			a-> parent -> right = b -> parent -> right;
			b-> parent -> right = swapinner;
		}
		else{
			swapinner = a-> parent -> right;
			a-> parent -> right = b-> parent -> left;
			b-> parent -> left = swapinner;
		}
	}
	if(a-> parent -> left ==a){
		if(b-> parent -> left == b){
			swapinner = a-> parent -> left;
...
// This function generates the post fix expression by using recursion. What happens is that we recursively search
// the left subtree until we reach the null pointer. We then recursively search the right subtree until the same outcome
// and load the respective expressions while indexing the nth pointer each time.
...
// Get total resource is a function that calculates the total amount of resources in all of the modules. This function uses
// recursion again to go through all leaf nodes and then add the resources to a rolling sum.
int get_total_resource(node_t* ptr)
//The function get_total_resource takes the root pointer
//and should return total sum of resource of modules(the variable resource in module_t) in the tree.
//You should traverse the tree to get this sum.
{
  int sum=0;
...
//At each recursion step, first generate an internal node with vertical cutline and a right child node attached by a module pointer
//(as the property of a left-skewed slicing tree). Then create a left child node and step to the next recursion parented by the current internal node.
//Establish the tree links appropriately (left, right, parent pointers).
//The recursion ceases when no more modules can be attached, and should ultimately return a pointer to the tree root.

// this function uses malloc to create the left skewed tree. We use recursion for the left side of the tree.
node_t* init_slicing_tree(node_t* par, int n) {
  	assert(n >= 0 && n < num_modules);
 	node_t *ptr;								//initialize a pointer to the struct node_t
...
if(n == num_modules - 1) {
  ...
  ptr -> left = NULL;
  ptr -> right = NULL;								         // assign the data to the module pointed by ptr
  return ptr;
}
  ptr -> cutline = V;
  ptr -> module = NULL;													// Internal node
...
ptr -> right -> left = NULL;
ptr -> right -> parent = ptr;														        //ptr->right <- malloc a new node of node_t as the right child of ptr
